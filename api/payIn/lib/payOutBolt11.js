...
export async function payOutBolt11Replacement (models, genesisId, { payOutType, userId, msats }, testBolt11, { keepRetryingWallets = false } = {}) {
  ...
  // keep retrying all wallets (least-failed first) — only a total absence of receive wallets ends it
  // (via createPayOutBolt11FromWalletProtocols' "no wallet to receive available").
