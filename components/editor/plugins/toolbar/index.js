...
function ToolbarDropdown ({ icon, tooltip, options, onAction, arrow = true, showDelay = 500, children }) {
  ...
  return (
    <ActionTooltip notForm overlayText={tooltip} placement='top' showDelay={showDelay} disable={dropdownOpen}>
      {/* no noWrapper: ActionTooltip's default span is the tooltip anchor (rb parity — it anchored to the Dropdown root span) */}
      <Menu open={dropdownOpen} onOpenChange={setDropdownOpen}>
        <Menu.Trigger
          ...
          onClick={() => setDropdownOpen(o => !o)} /* …so the click toggle drives open (pre-flight variant 3) */
...
function DropdownMenuItem ({ option, onAction, isActive }) {
  ...
  // raw BaseMenu.Item on the composed skins (login-button precedent): menuStyles.item
  // carries the dying globals item color/hover paint, the dropdownExtra* module
  // skins carry the toolbar-specific metrics; items close on click natively
  return (
    <BaseMenu.Item
      title={tooltipText}
...
function ToolbarButton ({ id, isActive, onClick, tooltip, children, showDelay = 500, composite = true }) {
  ...
  return (
    <ActionTooltip notForm overlayText={tooltipText} placement='top' noWrapper showDelay={showDelay}>
      <ButtonTag
        type={composite ? undefined : 'button'}
        title={tooltipText}
