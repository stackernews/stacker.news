...
export function useImgUrlCache (text) {
  const ref = useRef({})
...
export function ZoomableImage ({ src, topLevel, srcSet: srcSetObj, ...props }) {
  ...
  // if there is no srcset obj, image is still processing. we handle this the same as imgproxy errors
  ...
  if (!srcSetObj && originalUrlConsent && !originalErr) {
