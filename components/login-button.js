    ...
    <div className='inline-flex w-full mb-6'>
      ...
      {(accounts.length > 1 || !account) && (
        <Menu className='flex shrink-0'>
          ...
          {/* utilities carry the metrics — they'd beat the skin's declarations anyway
              (§11.0 inversion): w-40 = today's painted width (the old inline 150px lost
              to Bootstrap's 160px min-width), p-2 ≈ the skin's .45/.35rem, rounded-md ≈
              its .4rem. The skin lines go inert here but stay for the C8b toolbar */}
          <Menu.Popup align='end' className={cn(styles.dropdownExtra, 'w-40 p-2 rounded-md')}>
            {accounts.map(account => (
              // raw Base UI Item on the legacy dropdownExtra* skins, NOT itemClasses —
              // its padding utilities would out-!important the skin (§11.0, §14.5)
              <BaseMenu.Item
                key={account.id}
                onClick={() => {
