...
function Progress () {
  ...
  const style = (index) => {
    if (maxSteps === 2) {
      return { marginLeft: '-13px', marginRight: '-15px' }
    }
    // 3+ steps
    switch (index) {
      case 0: return { marginLeft: '-5px', marginRight: '-13px' }
      case 1: return { marginLeft: '-13px', marginRight: '-13px' }
      default: return { marginLeft: '-13px', marginRight: '-15px' }
    }
  }
  ...
  const Number = () => {
    ...
    switch (number) {
      ...
      case 4:
        return (
          <svg {...svgProps}>
            <path fill={numberColor} d='M16 1.5V12H18V14H16V22H14V14H5L14 1.5H16ZM14 12V5.963L8.063 12H14Z' />
          </svg>
        )
      default:
        return null
