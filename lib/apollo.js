...
export function mergeUniqueById (existing, incoming, readField) {
  const ids = new Set()
          ...
          fields: {
            ...
            items: {
              ...
              merge (existing, incoming, { readField }) {
                ...
                return {
                  cursor: incoming.cursor,
                  items: mergeUniqueById(existing?.items, incoming.items, readField),
            ...
            satistics: {
              keyArgs: ['walletId'],
              merge (existing, incoming) {
                if (isFirstPage(incoming.cursor, existing?.txs)) {
                  return incoming
                }

                return {
                  cursor: incoming.cursor,
                  txs: [...(existing?.txs || []), ...incoming.txs]
                }
              }
            },
            walletLogs: {
              keyArgs: ['walletId', 'payInId', 'externalTransactionId'],
              merge (existing, incoming) {
                return incoming
              }
            },
          ...
          fields: {
            paidAt: {
              read (existing, { readField }) {
                ...
                const payIn = readField('payIn')
                const isPaidItemCreate = payIn &&
                  readField('payInType', payIn) === 'ITEM_CREATE' &&
                  readField('payInState', payIn) === 'PAID'

                return isPaidItemCreate
                  ? readField('payInStateChangedAt', payIn) ?? null
                  : null
              }
            },
