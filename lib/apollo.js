        ...
        Item: {
          fields: {
            comments: {
              keyArgs: ['sort'],
              merge (existing, incoming) {
                // replace the cache if we're on the first page of a paginated comment section
                // or if the incoming comments are more than COMMENTS_LIMIT, meaning that comments are not being paginated.
                if (isFirstPage(incoming.cursor, existing?.comments, COMMENTS_LIMIT) || incoming.comments?.length >= COMMENTS_LIMIT) {
                  return incoming
                }
