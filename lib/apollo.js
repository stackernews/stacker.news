        ...
        Item: {
          fields: {
            comments: {
              keyArgs: ['sort'],
              merge (existing, incoming) {
                // replace the cache if the incoming comments are more than COMMENTS_LIMIT, meaning that comments are not being paginated
                // or if we're on the first page of a paginated comment section
                const isNotPaginated = !incoming.cursor && incoming.comments?.length > COMMENTS_LIMIT
                if (isNotPaginated || isFirstPage(incoming.cursor, existing?.comments, COMMENTS_LIMIT)) {
                  return incoming
                }
