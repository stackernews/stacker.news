import { ApolloClient, InMemoryCache, HttpLink, makeVar, ApolloLink } from '@apollo/client'
...
// preserve locally-bumped zap/boost/downzap counters against stale SSR writes on navigation.
// cache.modify (used by modifyActCache/updateAncestors) bypasses merge, so local bumps write
// through cleanly; merge only runs on writeQuery/query-result writes, which is where stale
// ssrData would otherwise clobber a pending zap that hasn't been sent to the server yet.
const maxMerge = (existing, incoming) => {
...
// notifications is the surface where a failed payment is seen and retried. its responses are
// written as truth: walk the (pre-merge) response and set each item's counters via cache.modify,
// which bypasses maxMerge — the one place a counter can move DOWN to match the server (a dead
// lineage the optimistic bump still shows, here or in another tab). This is what reverts a
// stale bump so a "failed" notification isn't sitting next to inflated item sats, and what keeps
// a retry's re-bump consistent. It is scoped to Notifications ONLY, and act mutations return a
// bare ItemAct (no item total), so an action never reconciles in others' deltas — that was the
// failure of the broader server-counts experiment, not of this link.
    ...
    cache: new InMemoryCache({
      ...
      typePolicies: {
        ...
        Query: {
          fields: {
            ...
            search: {
              ...
              merge (existing, incoming) {
                ...
                return {
                  cursor: incoming.cursor,
                  items: [...(existing?.items || []), ...incoming.items],
                  searchSuggestion: existing?.searchSuggestion ?? incoming.searchSuggestion
                }
              }
            },
            searchUsers: {
            ...
            satistics: {
              keyArgs: ['walletId'],
              merge (existing, incoming) {
                if (isFirstPage(incoming.cursor, existing?.payIns)) {
                  return incoming
                }

                return {
                  cursor: incoming.cursor,
                  payIns: [...(existing?.payIns || []), ...incoming.payIns]
                }
              }
            },
            walletLogs: {
              keyArgs: ['walletId', 'payInId'],
              merge (existing, incoming) {
                return incoming
              }
            },
        ...
        PayInBolt11: {
          fields: {
            hmac: {
              // never get rid of the hmac after it's been set
              merge (existing, incoming) {
                // XXX if we don't return null when there's no hmac, apollo assumes that any query
                // looking for this field is a cache miss and will refetch the
                // entire query
