...
import { COMMENTS_LIMIT, SSR, ITEM_EDIT_SECONDS } from './constants'
          ...
          fields: {
            ...
            nDirectComments: {
              merge (existing, incoming, { variables, readField, toReference }) {
                // if the comment (item) is still editable, don't merge the nDirectComments field
                // preventing new comments not yet injected from being counted, causing bottomedOut
                if (variables?.id && existing !== undefined) {
                  const item = toReference({
                    __typename: 'Item',
                    id: variables.id
                  })
                  const createdAt = readField('createdAt', item)
                  const editThreshold = datePivot(new Date(createdAt), { seconds: ITEM_EDIT_SECONDS })
                  // if we're still in the edit window, don't merge the nDirectComments field
                  if (Date.now() < editThreshold) {
                    return existing
                  }
                }
