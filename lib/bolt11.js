...
// Client-side BOLT11 parsing is for UX only. Server payment validation and
// bookkeeping use the singleton LND decoder.
...
export function bolt11ExpiresAt (bolt11) {
  return bolt11ExpiresAtFromDecoded(safeDecodeBolt11(bolt11))
}

// callers holding a memoized decode (e.g. render paths) skip the second parse
export function bolt11ExpiresAtFromDecoded (decoded) {
