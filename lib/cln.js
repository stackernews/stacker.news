...
import { WALLET_CREATE_INVOICE_TIMEOUT_MS, WALLET_SEND_PAYMENT_TIMEOUT_MS } from './constants'
...
export const sendPayment = async (bolt11, { socket, rune, cert }, { signal }) => {
  const scheme = process.env.NODE_ENV === 'development' ? 'http:' : 'https:'
  const url = new URL('/v1/pay', `${scheme}//${socket}`)
  ...
  const body = JSON.stringify({
    invstring: bolt11,
    retry_for: WALLET_SEND_PAYMENT_TIMEOUT_MS / 1000
  })

  const method = 'POST'
  const res = await fetchWithTimeout(url, { method, headers, body, signal })
  assertResponseOk(res, { method })
  assertContentTypeJson(res, { method })
