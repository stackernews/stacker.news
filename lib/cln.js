...
export const getPayment = async ({ paymentHash, bolt11 }, { socket, rune, cert }, { signal }) => {
  // an unfiltered listpays would return the node's whole payment history
  if (!paymentHash && !bolt11) throw new Error('payment hash or bolt11 required')
  const method = 'POST'
  const body = {}
...
export const sendPayment = async (bolt11, { socket, rune }, { signal, maxFee }) => {
  ...
  if (result?.status === 'failed') {
    return { status: 'FAILED', detail: 'cln reports payment failed' }
  }
  if (result?.status !== 'complete') return { status: 'PENDING' }
...
export function clnPaymentFeeMsats (payment) {
  ...
  // a negative derived fee passes through; the classifier rejects it
  return sent - amount
}
