...
export function injectComment (cache, rootId, newComment, { live = false } = {}) {
  // if live and a reply, check if the parent has comments
  const hasComments = live && !(Number(rootId) === Number(newComment.parentId))
    ? !!(cache.readFragment({
        id: `Item:${newComment.parentId}`,
        fragment: HAS_COMMENTS
      }))
    // if not live, assume the parent has comments, otherwise how did we reply to it?
    : true
  ...
  if (updated || (live && !hasComments)) {
    ...
    updateAncestorsCommentCount(cache, ancestors, newComment.parentId)
