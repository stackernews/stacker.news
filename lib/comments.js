...
export function writeItemQuery (cache, id, sort, data) {
  cache.writeQuery({
    query: ITEM_FULL,
    variables: sort ? { id, sort } : { id },
    data: { item: data }
...
export function writeCommentFragment (cache, id, data) {
  let result = cache.writeFragment({
    id: `Item:${id}`,
    fragment: COMMENT_WITH_NEW_RECURSIVE,
    fragmentName: 'CommentWithNewRecursive',
    data
  ...
  // at the deepest level, the comment can't have any children, here we update only the stats fields.
  if (!result) {
    result = cache.writeFragment({
      id: `Item:${id}`,
      fragment: COMMENT_WITH_NEW_MINIMAL,
