import { COMMENT_WITH_NEW_RECURSIVE, COMMENT_WITH_NEW_LIMITED, COMMENT_WITH_NEW_MINIMAL, COMMENTS } from '../fragments/comments'
import { ITEM_FULL } from '../fragments/items'

export function injectComment (cache, newComment, { live = false } = {}) {
  const updated = cache.modify({
  ...
  if (updated) {
    ...
    const itemHierarchy = newComment.path.split('.')
    // update all ancestors comment count, but not the item itself
    const ancestors = itemHierarchy.slice(0, -1)
    updateAncestorsCommentCount(cache, ancestors, totalNComments, newComment.parentId)
  }

  return newComment.createdAt
}
