...
// proxy payer bounds advertised via lnurlp and enforced by its callback. both are
// whole-sat multiples because a payIn's mcost must be a multiple of 1000:
// min: smallest whole-sat amount whose (100 - PROXY_RECEIVE_FEE_PERCENT)% payout still floors to >= 1 sat
...
export const FASTISH_POLL_INTERVAL_MS = Number(process.env.NEXT_PUBLIC_FASTISH_POLL_INTERVAL_MS)
...
export const WALLET_EXTERNAL_TX_CHECK_TIMEOUT_MS = 10_000
// An accepted HTLC can outlive its invoice. Thirty days covers the expected
// maximum HTLC lifetime plus provider indexing time.
export const RECONCILIATION_GRACE_MS = 30 * 24 * 60 * 60_000
export const WALLET_MAX_PENDING_PAYOUT_INVOICES = 25
export const WALLET_MAX_PENDING_EXTERNAL_RECEIVES = 25
export const WALLET_EXTERNAL_SEND_POLL_AFTER_MS =
  WALLET_SHELL_SEND_PAYMENT_TIMEOUT_MS + WALLET_EXTERNAL_TX_CHECK_TIMEOUT_MS + 5_000
