...
export default function getGetCsvServerSideProps (query, variables = null) {
  return async function ({ req, res, query: params }) {
    ...
    res.write(generateCsvHeader())

    do {
      ({ error, data: { walletHistory: { cursor, facts } } } = await client.query({
        query, variables: { cursor, ...params, ...variables }
      }))
      res.write(facts?.map(generateCsvItem).join(''))
    } while (cursor && !error)

    if (error) return emptyProps
