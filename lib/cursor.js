...
export function decodeCursor (cursor) {
  if (!cursor) {
    return { offset: 0, time: new Date(), ids: [] }
  } else {
    const res = JSON.parse(Buffer.from(cursor, 'base64'))
    res.offset = Number(res.offset)
    res.time = new Date(res.time)
    res.ids = Array.isArray(res.ids) ? res.ids.map(id => Number(id)).filter(Number.isInteger) : []
    return res
  }
}

export function nextCursorEncoded (cursor, limit = LIMIT, ids = []) {
  const nextCursor = { ...cursor }
  nextCursor.offset += limit
  nextCursor.ids = [...new Set([...(cursor.ids || []), ...ids])]
