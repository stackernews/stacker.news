...
export const ACM_TERMINAL_FAILED_STATUSES = new Set([
  'FAILED',
...
const CONCLUSIVE_DNS_ERROR_CODES = new Set(['ENOTFOUND', 'ENODATA'])
...
export async function verifyDNSRecord (type, recordName, recordValue) {
  ...
  try {
    ...
    return { valid: false, error, transient: true }
  }
  ...
  try {
    ...
    if (CONCLUSIVE_DNS_ERROR_CODES.has(error.code)) {
      result.error = { message: `DNS record not found: ${error.code}`, code: error.code }
    } else {
      // resolver/network/unknown are transient errors
      result.transient = true
      result.error = { message: `DNS resolver error: ${error.code || error.message}`, code: error.code }
    }
  }
