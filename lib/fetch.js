...
export async function snFetch (url, { path, protocol = 'https', timeout = 10000, cert, agent = true, signal, ...options } = {}) {
  ...
  // TODO/ADDRESS BEFORE REVIEW: dynamic import to avoid pulling Node.js 'http' into the edge runtime via middleware
  const fetchAgent = isServer && agent !== false
    ? (await import('@/lib/proxy')).getAgent({ hostname: urlObj.hostname, cert, protocol: urlObj.protocol })
    : undefined
