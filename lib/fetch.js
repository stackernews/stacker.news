...
export async function snFetch (url, { path, protocol = 'https', timeout = 10000, cert, agent = true, signal, ...options } = {}) {
  ...
  // join path properly using URL constructor (handles trailing slashes)
  const urlObj = path ? new URL(path, urlStr) : new URL(urlStr)
  ...
  const fetchAgent = isServer && agent !== false ? getAgent({ hostname: urlObj.hostname, cert }) : undefined
