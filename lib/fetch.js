import { isAbortLike, TimeoutError, withTimeoutSignal } from '@/lib/time'
...
export async function snFetch (url, { path, protocol = 'https', timeout = 10000, cert, agent = true, allowPrivate = false, signal, ...options } = {}) {
  ...
  // only create a timeout signal if no signal provided; the wrapper clears the
  // timer once headers resolve, so it does not cover body streaming
  if (signal) return await doFetch(signal)
  try {
    return await withTimeoutSignal(timeout, doFetch)
  } catch (err) {
