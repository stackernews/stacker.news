import { isAbortLike, TimeoutError, timeoutSignal } from '@/lib/time'
...
export async function snFetch (url, { path, protocol = 'https', timeout = 10000, cert, agent = true, allowPrivate = false, signal, ...options } = {}) {
  ...
  const doFetch = fetchSignal => crossFetch(urlObj.toString(), {
  ...
  // callers that pass their own signal own their own bounding
  if (signal) return await doFetch(signal)
  // no caller signal: bound the WHOLE exchange, headers and body. A scope-owning
  // wrapper would disarm at headers and let a slow-dripped body hang the caller's
  // res.text() forever; the raw timeout signal self-cleans when it fires or aborts.
  const { signal: deadline } = timeoutSignal(timeout)
  try {
    return await doFetch(deadline)
  } catch (err) {
