...
// Coerce numeric strings so the strict singular check works, but preserve
// nonnumeric values for numWithUnits' existing fallback instead of displaying NaN.
export const formatSats = (sats) => {
  const n = Number(sats)
  return numWithUnits(Number.isNaN(n) ? sats : n, { unitSingular: 'sat', unitPlural: 'sats', abbreviate: false })
}
export const formatMsats = (msats) => numWithUnits(toPositiveNumber(msats), { unitSingular: 'msat', unitPlural: 'msats', abbreviate: false })
// format a msat amount as display sats ("1 sat", "1.5 sats"); undefined for a nullish amount so call
// sites don't each need an `x != null ?` guard
export const formatMsatsToSats = msats => msats == null ? undefined : formatSats(msatsToSatsDecimal(msats))
export const formatMsatsToCCs = msats => msats == null
  ? undefined
  : numWithUnits(Number(msatsToSatsDecimal(msats)), { unitSingular: 'CC', unitPlural: 'CCs', abbreviate: false })
