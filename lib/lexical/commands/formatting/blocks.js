...
export function $formatBlock (editor, block) {
  ...
  const activeBlock = $snGetBlockType(editor, selection)
...
export function registerSNFormatBlockCommand ({ editor }) {
  return editor.registerCommand(SN_FORMAT_BLOCK_COMMAND, (block) => {
    ...
    console.log('markdown mode formatting via transformer bridge', block)
    if (!$isRangeSelection(selection) || selection.isCollapsed()) return handleMDInsertBlockCommand({ type: block })
