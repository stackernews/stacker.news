import { createCommand, COMMAND_PRIORITY_HIGH, $getSelection, $isRangeSelection } from 'lexical'
...
const FORMAT_HANDLERS = {
  ...
  underline: () => $wrapMarkdownSelection('<ins>', '</ins>'),
  strikethrough: () => $wrapMarkdownSelection('~~'),
  inlineMath: () => $wrapMarkdownSelection('$$'),
  math: () => $wrapMarkdownSelection('$$\n', '\n$$', 3)
}

const BLOCK_HANDLERS = {
  h1: () => $prefixEachLine((line) => `# ${line}`),
  h2: () => $prefixEachLine((line) => `## ${line}`),
  h3: () => $prefixEachLine((line) => `### ${line}`),
  number: () => $prefixEachLine((line, index) => `${index + 1}. ${line}`),
  check: () => $prefixEachLine((line) => `- [ ] ${line}`),
  bullet: () => $prefixEachLine((line) => `- ${line}`),
  quote: () => $prefixEachLine(line => `> ${line}`),
  code: () => handleCodeblockCommand()
}
...
function handleMDInsertBlockCommand (type) {
  return BLOCK_HANDLERS[type]?.() ?? false
}

export function registerMDFormatCommand (editor) {
