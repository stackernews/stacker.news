...
export function registerSNToggleLinkCommand (editor) {
  return editor.registerCommand(SN_TOGGLE_LINK_COMMAND, (url) => {
    ...
    // transformer bridge
    const handled = editor.dispatchCommand(USE_TRANSFORMER_BRIDGE, { formatType: 'link', transformation: url })

    // check if selection already contains a markdown link
    const markdown = selection.getTextContent()
    // will select the URL placeholder if we toggled a link ON
    if (handled && !hasMarkdownLink(markdown)) {
