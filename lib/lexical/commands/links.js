...
function $toggleLinkCommand (editor, url) {
  ...
  if (!markdownMode) return $toggleLink(editor, linkUrl)
  ...
  // transformer bridge
  const handled = editor.dispatchCommand(USE_TRANSFORMER_BRIDGE, { formatType: 'link', transformation: linkUrl })
  if (!handled || hasMarkdownLink(markdown)) return handled
  ...
  if (linkUrl !== DEFAULT_URL) { // select the alt text inside the square brackets
    $moveSelection(updatedSelection, -(linkUrl.length + linkUrl.length + 3), -(linkUrl.length + 3))
  } else { // select the URL placeholder inside the parenthesis
