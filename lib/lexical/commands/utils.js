...
/**
 * checks if the editor is in markdown mode by checking the namespace of the current editor
 *
 * this function must be called inside a read/update transaction
 *
 * @returns {boolean} true if the editor is in markdown mode
 */
export function $isMarkdownMode () {
  return isMarkdownMode($getEditor())
}
