    ...
    return editor.registerCommand(
      ...
      (event) => {
        ...
        for (let i = 0; i < children.length; i++) {
          ...
          let nearTop = y >= rect.top - ZONE_SIZE && y < rect.top + ZONE_SIZE
          let nearBottom = y > rect.bottom - ZONE_SIZE && y <= rect.bottom + ZONE_SIZE
          if (!nearTop && !nearBottom) continue

          // when both zones overlap, prefer the closer zone
          // if the distances are equal, top winds as a tiebreaker
      ...
      // priority is NORMAL to win against other click handlers
      // this is because we want to ensure the user can insert paragraphs at all times
      // priority must be reconsidered when we introduce more click handlers
      COMMAND_PRIORITY_NORMAL
    )
  }
})
