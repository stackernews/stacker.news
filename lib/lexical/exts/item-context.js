...
/**
 * computes srcSet, bestResSrc, width, height from the srcSet object (imgproxyUrls[url])
 * @param {Object} srcSetObj
 * @param {string} src - original source URL
 * @returns {Object} { srcSet, bestResSrc, width, height }
 */
const processSrcSetInitial = (srcSetInitial, src) => {
  if (!srcSetInitial) {
    return { srcSet: null, bestResSrc: src, width: 0, height: 0 }
  }
