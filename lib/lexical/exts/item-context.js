  ...
  register: (editor, { imgproxyUrls, outlawed, rel, showImagesAndVideos, imgproxyOnly } = {}) => {
    ...
    const unregisterImgproxyUrls = editor.registerNodeTransform(MediaNode, (node) => {
      ...
      if (imgproxyUrls) {
        ...
        // we can't proxy videos, if the user has disabled non-proxied media, replace the media node with a link
        if (imgproxyOnly && video) $replaceNodeWithLink(node, src)
      ...
      } else if (imgproxyOnly) { // no imgproxy urls
        // if the user has disabled non-proxied media, replace the media node with a link
        $replaceNodeWithLink(node, src)
      }
    })
