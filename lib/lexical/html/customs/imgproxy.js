/**
 * adds srcset attributes to images and poster to videos using imgproxy URLs
 * @param {Document} doc - document object
 * @param {Object} imgproxyUrls - map of original URLs to imgproxy URL sets
 * @param {boolean} topLevel - whether content is top-level (affects sizes attribute)
 */
export function addSrcSetToMediaAndVideoNodes (doc, imgproxyUrls, topLevel) {
  ...
  body.querySelectorAll('video[src]').forEach(videoEl => {
    ...
    if (srcSetObj && Object.keys(srcSetObj).length > 0) {
      ...
      videoEl.setAttribute('poster', bestResSrc !== src ? bestResSrc : '')
      videoEl.setAttribute('preload', bestResSrc !== src ? 'metadata' : '')
    }
  })
}
