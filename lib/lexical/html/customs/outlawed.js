...
function replaceMedia (doc) {
  const body = doc.body
  body.querySelectorAll('span.sn-media-container').forEach(mediaContainer => {
    const media = mediaContainer.querySelector('img,video,iframe,embed')
    const src = media?.getAttribute('src') || ''
    const p = doc.createElement('p')
    p.className = 'sn-paragraph outlawed'
    p.textContent = src

    // replace the mediaContainer with the new paragraph
    const parentParagraph = mediaContainer.closest('.sn-paragraph')
    if (parentParagraph) {
...
function replaceEmbeds (doc) {
  const body = doc.body
  body.querySelectorAll('div.sn-embed-wrapper__explainer').forEach(embed => {
    const src = embed.getAttribute('data-lexical-embed-src') || ''
    const p = doc.createElement('p')
    p.className = 'sn-paragraph outlawed'
    p.textContent = src
    const parentParagraph = embed.closest('.sn-paragraph')
    if (parentParagraph) {
      parentParagraph.replaceWith(p)
    } else {
      embed.replaceWith(p)
