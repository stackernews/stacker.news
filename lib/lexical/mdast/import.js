...
export function importMarkdownToLexical ({
  ...
  if (!transformerBridge && mdastRoot.children.at(-1)?.type !== 'paragraph') {
    mdastRoot.children.push({ type: 'paragraph', children: [] })
  }

  importMdastTreeToLexical({ root, mdastRoot, visitors, markdown, transformerBridge, ...descriptors })
}
