...
const isMdastList = (node) => node.type === 'list'

export const MdastListItemVisitor = {
  testNode: 'listItem',
  visitNode ({ mdastNode, actions }) {
    const checked = typeof mdastNode.checked === 'boolean' ? mdastNode.checked : null
    const contentChildren = mdastNode.children.filter((child) => !isMdastList(child))
    const nestedListChildren = mdastNode.children.filter(isMdastList)

    if (contentChildren.length > 0 || nestedListChildren.length === 0) {
      const listItemNode = $createListItemNode(checked)
      actions.addAndStepInto(listItemNode, { children: contentChildren })
    }

    nestedListChildren.forEach((child) => {
      const wrapperListItemNode = $createListItemNode()
      actions.addAndStepInto(wrapperListItemNode, { children: [child] })
    })
  }
}
