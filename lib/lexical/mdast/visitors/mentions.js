...
import { $createItemMentionNode, isCustomText, formatChildrenToMdast } from '@/lib/lexical/nodes/decorative/mentions/item'
...
export const LexicalItemMentionVisitor = {
  ...
  visitLexicalNode ({ lexicalNode, mdastParent, actions }) {
    ...
    if (isCustomText(text, id)) {
      // export as a LinkNode, preserving any text formatting
      actions.appendToParent(mdastParent, {
        type: 'link',
        url,
        children: formatChildrenToMdast(text, lexicalNode.getFormat())
      })
    } else {
      // export as text node
      actions.appendToParent(mdastParent, {
