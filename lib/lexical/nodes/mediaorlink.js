import { DecoratorNode, $applyNodeReplacement } from 'lexical'
import MediaOrLink from '@/components/media-or-link'
import { IMG_URL_REGEXP, VIDEO_URL_REGEXP } from '@/lib/url'
import { UNKNOWN_LINK_REL } from '@/lib/constants'

// Recognize if the link is a media or a link to a page
export class MediaOrLinkNode extends DecoratorNode {
  __src
  __rel
  __linkFallback

  static getType () {
    return 'mediaOrLink'
  }

  getSrc () {
    return this.__src
  }

  getAltText () {
    return ''
  }

  getInnerType () {
    if (IMG_URL_REGEXP.test(this.__src)) {
      return 'image'
    }
    if (VIDEO_URL_REGEXP.test(this.__src)) {
      return 'video'
    }
    return 'link'
  }

  static clone (node) {
    return new MediaOrLinkNode(
      node.__src,
      node.__rel,
      node.__linkFallback,
      node.__key
    )
  }

  static importJSON (serializedNode) {
    const { src, rel, linkFallback } = serializedNode
    return $createMediaOrLinkNode({ src, rel, linkFallback })
  }

  exportJSON () {
    return {
      type: 'mediaOrLink',
      version: 1,
      src: this.__src,
      rel: this.__rel,
      linkFallback: this.__linkFallback
    }
  }

  static importDOM () {
    return {
      a: () => ({
        conversion: (node) => {
          if (node instanceof window.HTMLAnchorElement) {
            const href = node.getAttribute('href')
            if (!href) return null
            const rel = node.getAttribute('rel') || UNKNOWN_LINK_REL
            return { node: $createMediaOrLinkNode({ src: href, rel }) }
          }
          return null
        },
        priority: 1
      })
    }
  }

  exportDOM () {
    const src = this.__src
    const innerType = this.getInnerType()
    let element
    switch (innerType) {
      case 'image':
        element = document.createElement('img')
        element.setAttribute('src', src)
        element.setAttribute('alt', '')
        break
      case 'video':
        element = document.createElement('video')
        element.setAttribute('src', src)
        element.setAttribute('controls', '')
        break
      default:
        element = document.createElement('a')
        element.setAttribute('href', src)
        if (this.__rel) element.setAttribute('rel', this.__rel)
        break
    }
    return { element }
  }

  constructor (src, rel, linkFallback = true, key) {
    super(key)
    this.__src = src
    this.__rel = rel
    this.__linkFallback = linkFallback
  }

  createDOM () {
    return document.createElement('span')
  }

  updateDOM () {
    return false
  }

  decorate () {
    return (
      <MediaOrLink
        src={this.__src}
        rel={this.__rel}
        linkFallback={this.__linkFallback}
      />
    )
  }
}

export function $createMediaOrLinkNode ({ src, rel, linkFallback = true, key }) {
  return $applyNodeReplacement(
    new MediaOrLinkNode(src, rel, linkFallback, key)
  )
}

export function $isMediaOrLinkNode (node) {
  return node instanceof MediaOrLinkNode
}
