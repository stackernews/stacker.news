...
export function lexicalStateLoader ({ me, userLoader } = {}) {
  ...
  return new DataLoader(
    async (keys) => {
      const satsFilter = await getSatsFilter()
      return Promise.all(
        keys.map(({ text, context }) => {
          const { userId, parentId, netInvestment, ...rest } = context || {}
          let outlawed = false
          if (netInvestment !== undefined) {
            const threshold = parentId ? satsFilter.commentsSatsFilter : satsFilter.postsSatsFilter
            outlawed = me?.id !== userId && (netInvestment ?? 0) < threshold
          }
          return prepareLexicalState({ text, context: { ...rest, outlawed } })
            .catch(error => {
