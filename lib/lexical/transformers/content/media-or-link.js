...
export const MEDIA_OR_LINK = {
  ...
  export: (node) => {
    ...
    if ($isMediaNode(node)) {
      let newText = `![${node.getAltText() || ''}](${node.getSrc()})`
      if (node.getShowCaption()) {
        const caption = node.getCaptionText()
        const escapedCaption = escapeText(caption)
        if (escapedCaption) {
          newText = `![${node.getAltText() || escapedCaption}](${node.getSrc()} "${escapedCaption}")`
        }
      }
      if (node.getWidthAndHeight()) {
        const { width, height } = node.getWidthAndHeight()
        newText += `{width:${width},height:${height}}`
        return newText
      }
      return newText
    }
