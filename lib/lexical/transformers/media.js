...
import { $isImageNode, $createImageNode, ImageNode } from '@/lib/lexical/nodes/media/imagenode'
...
export const MEDIA_OR_LINK = {
  dependencies: [LinkNode, ImageNode, MediaOrLinkNode, ...EmbedNodes],
  export: (node) => {
  ...
  replace: (textNode, match) => {
    ...
    if (innerType.provider) {
      ...
      newNode = $createImageNode({ src: url })
