...
export async function fetchLnAddrService (addr, { signal } = {}) {
  ...
  const body = await parseLnurlBody(res, { method, url, requiredField: 'callback' })
...
// https://github.com/lnurl/luds/blob/luds/01.md#L36
async function parseLnurlBody (res, { method, url, requiredField }) {
  let body
  try {
    body = await res.json()
  } catch (err) {
    // a timeout/abort during the body read is our doing, not provider corruption
    if (isAbortLike(err)) throw err
    assertResponseOk(res, { method })
    assertContentTypeJson(res, { method })
    throw new Error(`GET ${url}: invalid JSON`, { cause: err })
  }
  if (body?.status !== 'ERROR' && body?.[requiredField] == null) {
    assertResponseOk(res, { method })
  }
  return body
}
