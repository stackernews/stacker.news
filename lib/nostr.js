...
export class NostrSigner {
  ...
  async subscribeMany (relays, filters, handlers) {
    this.connectedRelays.push(relays.filter(r => !this.connectedRelays.includes(r)))
    return this.pool.subscribeMany(relays, filters, handlers)
  }
  ...
  disconnectApp () {
    ...
    // reset auth state
    this.nip46Auth = false
    // TODO: this seems to cause 'websocket already in CLOSING state' downstream, possible nostr-tools bug
  ...
  stopListeningForSpontaneousConnections () {
    ...
    // const relays = this.bunker && this.bunker.relays ? this.appRelayUrls.filter(r => !this.bunker.relays.includes(r)) : this.appRelayUrls
    // this.pool.close(relays)
  }
  ...
  close () {
    ...
    this.pool.close(this.connectedRelays)
    this.connectedRelays = []
  }
