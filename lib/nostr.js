      ...
      new Promise(
        (resolve, reject) => {
          ...
          function onmessage (msg) {
            const [type, ackId, accepted, reason] = JSON.parse(msg.data)

            if (type !== 'OK' || ackId !== id) return

            if (accepted) {
              resolve(id)
            } else {
              reject(new Error(reason || `event rejected: ${id}`))
            }
          }

          listeners = [['message', onmessage]]
          listeners.forEach(l => ws.addEventListener(...l))
        })
        .finally(() => {
          listeners.forEach((l) => ws.removeEventListener(...l))
        })
    ])
  }
    ...
    return await Promise.race([
      ...
      new Promise(
        (resolve, reject) => {
          ...
          function onmessage (msg) {
            const [type, ackId, event] = JSON.parse(msg.data)
