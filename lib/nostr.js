...
export class Relay {
  constructor (relayUrl, { reconnect } = {}) {
    this.url = relayUrl
    this.subs = []
    this._closed = false
  ...
  reconnect (reconnectOnClose) {
    ...
    ws.onclose = () => {
      if (reconnectOnClose && !this._closed) {
        setTimeout(() => {
          if (!this._closed) {
            this.reconnect(reconnectOnClose)
          }
        }, 5000)
      }
    }
  ...
  static async connect (url, { timeout, reconnect } = {}) {
    const relay = new Relay(url, { reconnect })
...
export class NostrSigner {
  ...
  async subscribeMany (relaysUrls, filters, handler) {
    let newRelays = []
    const connectedRelays = []
    for (const relayUrl of relaysUrls) {
      const relay = this.pool.find(p => p.url === relayUrl)
      if (relay) {
        connectedRelays.push(relay)
      } else {
        newRelays.push(Relay.connect(relayUrl, { reconnect: true }))
      }
    }
    newRelays = await Promise.allSettled(newRelays)
    for (const newRelay of newRelays) {
      if (newRelay.status === 'fulfilled') {
        connectedRelays.push(newRelay.value)
        this.pool.push(newRelay.value)
      } else {
        console.error('failed to connect to relay:', newRelay.reason)
      }
    }
    const subs = []
    for (const relay of connectedRelays) {
      const sub = relay.subscribe(filters, handler)
      subs.push(sub)
    }
    return subs
  }
  ...
  async _onIncomingConnection (event, challengeHandler, connectionRequestHandler) {
    if (this.nip46Auth) return // already connected
    const userPubKey = event.content.params[0] || event.pubkey
    if (connectionRequestHandler) {
      if (!await connectionRequestHandler(userPubKey)) {
        console.log('connection request from', userPubKey, 'was rejected')
        return false
      }
    }
   ...
   */
  waitForResponse (id, method, challengeHandler) {
   ...
   * @param {*} connectionRequestHandler - A callback that is called when a connection request is received, returns true if the connection is accepted
   * @param {*} onConnection - A callback that is called when a connection is established
   */
  async startListeningForSpontaneousConnections (challengeHandler, connectionRequestHandler, onConnection) {
    try {
      ...
      async (event) => {
        try {
          ...
          if (method === 'connect') {
            ...
            this.enqueue(async () => {
              const res = await this._onIncomingConnection(event, challengeHandler, connectionRequestHandler)
