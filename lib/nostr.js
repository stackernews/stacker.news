...
export const RELAYS_WHITELIST = undefined
...
export class Nostr {
  ...
  constructor ({ privKey, signer, relays, ...ndkOptions }) {
    this.ndk = new NDK({
      ...
      devWriteRelayUrls: RELAYS_WHITELIST,
  ...
  async nwc (nwcUrl, { logger }) {
    ...
    if (logger) {
      ...
      debug.log(`Starting with wallet service ${nwcUrl}`)
      nwc.debug = debug
    }
    return nwc
  }
   ...
   * @param {number} rawEvent.createdAt
   ...
   */
  async sign ({ kind, createdAt, content, tags }, { privKey, signer, signWithExtension }) {
   ...
   * @returns {Promise<NDKEvent>}
   */
  async publish ({ createdAt, content, tags = [], kind }, { relays, privKey, signer, timeout, signWithExtension } = {}) {
    const event = await this.sign({ kind, createdAt, content, tags }, { privKey, signer, signWithExtension })

    const failedRelays = []
    const relaySet = NDKRelaySet.fromRelayUrls(relays, this.ndk, true)
    relaySet.on('publish:failed', (relay, error) => {
      failedRelays.push({ relay, error })
    })

    const successfulRelays = await relaySet.publish(event, timeout)
  ...
  async crosspost ({ createdAt, content, tags = [], kind }, { relays = DEFAULT_CROSSPOSTING_RELAYS, privKey, signer, timeout }) {
    try {
      const { event: signedEvent, successfulRelays, failedRelays } = await this.publish({ createdAt, content, tags, kind }, { relays, privKey, signer, timeout, signWithExtension: true })
