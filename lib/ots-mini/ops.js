...
import { MAX_TIMESTAMP_MSG_LENGTH } from './constants.js'
...
export class Op {
  /**
   * The maximum length in bytes of the message for this op
   * @returns {number}
   */
  _MAX_MSG_LENGTH () {
    return MAX_TIMESTAMP_MSG_LENGTH
  }
  ...
  static deserialize (ctx) {
    ...
    if (tag === OpSHA256.TAG) {
      ...
      return new OpAppend(ctx.readVarbytes(MAX_TIMESTAMP_MSG_LENGTH))
    } else if (tag === OpPrepend.TAG) {
      return new OpPrepend(ctx.readVarbytes(MAX_TIMESTAMP_MSG_LENGTH))
    }
...
export class CryptOp extends OpUnary {
  ...
  call (msg) {
    if (!(msg instanceof Buffer)) {
      throw new Error('CryptOp.call: msg must be a Buffer.')
    }
