...
export class Timestamp {
   ...
   * Otherwise: childMsg = op.call(this.msg), create new child Timestamp,
   * store in this.ops, return child.
   * Op key equivalence: compare by _TAG() and, for binary ops, by arg bytes.
   */
  add (op) {
   ...
   * - If the set grows past MAX_ATTESTATIONS_PER_TIMESTAMP, an error is thrown
   */
  addAttestation (attn) {
   ...
   * - Union attestation lists.
   * - For each op in other.ops:
   *     if this.ops has the same op key: recurse merge into the child
   *     else: deep-clone the subtree from other and add to this.ops
   */
  merge (other) {
   ...
   * Returns the populated Timestamp.
   */
...
export class DetachedTimestampFile {
   ...
   * hash: Buffer (must match fileHashOp._DIGEST_LENGTH())
   */
  ...
  /** Parse a .ots file from a Uint8Array. */
