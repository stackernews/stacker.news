...
import {
  HEADER_MAGIC,
  MAJOR_VERSION,
  MAX_ATTESTATIONS_PER_TIMESTAMP,
  TIMESTAMP_RECURSION_LIMIT,
  MAX_TIMESTAMP_MSG_LENGTH
} from './constants.js'
...
export class Timestamp {
   ...
   */
  constructor (msg) {
    if (!(msg instanceof Buffer) || msg.length < 1 || msg.length > MAX_TIMESTAMP_MSG_LENGTH) {
      throw new Error(`Timestamp(): message must be a buffer between 1 and ${MAX_TIMESTAMP_MSG_LENGTH} bytes long`)
    }
    this.msg = msg // Buffer
  ...
  /**
   * Add a prepared child
   * @param {Op} op
   * @param {Timestamp} child
   * @throws {Error} if a child with the same Op.uniqueId() exists
   */
  addChild (op, child) {
    const stamp = this.getStamp(op)
    if (stamp instanceof Timestamp) {
      throw new Error('Timestamp.addChild: Cannot add the same op twice to a single timestamp')
    }
    this.ops.set(op, child)
  }
   ...
   * @throws {Error} when the message of the merging stamp does not equal ours
   */
  merge (other) {
   ...
   * @throws {Error} when the TIMESTAMP_RECURSION_LIMIT is exceeded
   */
  serialize (ctx, recursions = 0) {
    if (recursions > TIMESTAMP_RECURSION_LIMIT) {
      throw new Error(`Timestamp.serialize: exceeded recursion limit of ${TIMESTAMP_RECURSION_LIMIT}`)
    }
    ...
    if (this.ops.size === 0 && sortedAttestations.length > 0) {
      ...
      this.ops.forEach((stamp, op) => {
        ...
        stamp.serialize(ctx, recursions + 1) // recurse into child timestamp
      })
    }
  }
   ...
   * @throws {Error} when the TIMESTAMP_RECURSION_LIMIT is exceeded
   */
  static deserialize (ctx, msg, recursions = 0) {
    if (recursions > TIMESTAMP_RECURSION_LIMIT) {
      throw new Error(`Timestamp.deserialize: exceeded recursion limit of ${TIMESTAMP_RECURSION_LIMIT}`)
    }
    ...
    function deserializeOp (ctx) {
      ...
      const child = Timestamp.deserialize(ctx, op.call(msg), recursions + 1)
      stamp.addChild(op, child)
    }
...
export class DetachedTimestampFile {
  ...
  static deserialize (data) {
    ...
    ctx.readVaruint(MAJOR_VERSION, MAJOR_VERSION)
    ...
    if (!ctx.atEOF()) {
      throw new Error('DetachedTimestampFile.deserialize: trailing data')
    }
