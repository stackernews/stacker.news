...
export const shouldSuppressPayInTimeoutToast = (err) => {
  ...
  if (Number(statusCode) === 504) return true

  const message = err?.networkError?.message ?? err?.message
  return /^Response not successful: Received status code 504$/i.test(message ?? '')
}
