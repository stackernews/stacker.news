...
import { shouldSuppressPayInTimeoutToast } from './pay-in'
import { TimeoutError } from './time'

describe('shouldSuppressPayInTimeoutToast', () => {
  test.each([
    ...
    expect(shouldSuppressPayInTimeoutToast(err)).toBe(true)
  })
  ...
  test.each([
    ...
    expect(shouldSuppressPayInTimeoutToast(err)).toBe(false)
  })
})
