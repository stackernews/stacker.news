...
export function getAgent ({ hostname, cert, protocol, lookup }) {
  ...
  if (isOnion) {
    ...
    const onionProtocol = cert ? 'https:' : 'http:'
    // we need to use our Tor proxy to resolve onion addresses,
    // so a custom lookup doesn't apply here
    const proxyOptions = { proxy: process.env.TOR_PROXY }
    return onionProtocol === 'https:'
      ? new HttpsProxyAgent({ ...proxyOptions, ...httpsAgentOptions, rejectUnauthorized: false })
      : new HttpProxyAgent(proxyOptions)
  }

  // return agent matching the URL's protocol,
  // resolving the hostname with lookup if provided
  if (protocol === 'http:') {
    return new http.Agent(lookup && { lookup })
  }

  return new https.Agent({ ...httpsAgentOptions, ...(lookup && { lookup }) })
}
