  ...
  return function transformer (tree) {
    try {
      ...
      visit(tree, (node, index, parent) => {
        // Handle details tag collection
        if (node.type === 'raw') {
          if (node.value === '<details>') {
            collectingDetails = true
            detailsContent = [node.value]
            return
          }
          
          if (node.value === '</details>' && collectingDetails) {
            collectingDetails = false
            detailsContent.push(node.value)
            // Log the collected content for debugging
            console.log('Collected details content:', detailsContent.join(''))
            return
          }
        }

        // Collect content while inside details
        if (collectingDetails) {
          if (node.type === 'text') {
            ...
            // Handle element nodes (like paragraphs)
            // This is a simplified version - we may need to handle nested elements
            if (node.children) {
              const elementContent = node.children.map(child => {
                if (child.type === 'text') return child.value
                if (child.type === 'raw') return child.value
                return ''
              }).join('')
              detailsContent.push(elementContent)
            }
          }
        }
