...
export function proxyEmbedUrl (href, provider) {
  ...
  if (provider === 'twitter') {
    const service = 'xcancel.com'
    return {
      provider,
      service,
      href: href.replace(/^(https?:\/\/)(twitter|x)\.com/, `$1${service}`)
    }
  }
...
export function parseEmbedUrl (href) {
  ...
  try {
    const twitter = href.match(TWITTER_URL_REGEXP)
...
export const TWITTER_URL_REGEXP = /^https?:\/\/(?:twitter|x)\.com\/(?:#!\/)?\w+\/status(?:es)?\/(?<id>\d+)/
