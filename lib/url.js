...
export function isExternal (url) {
  ...
  if (url.startsWith(process.env.NEXT_PUBLIC_URL + '/')) return false
...
const LINK_URL_PROTOCOLS = new Set(['http:', 'https:', 'mailto:', 'sms:', 'tel:'])
const MEDIA_URL_PROTOCOLS = new Set(['http:', 'https:'])

/**
 * collapses a URL to about:blank unless its scheme is allowed.
 * relative URLs keep their form, scheme is always checked with URL parser.
 * @param {string} url
 * @param {Set<string>} allowedProtocols - default to LINK_URL_PROTOCOLS
 * @returns {string} the original url, or 'about:blank'
 */
export function sanitizeUrl (url, allowedProtocols = LINK_URL_PROTOCOLS) {
  if (!url) return ''
  try {
    const parsed = new URL(url, process.env.NEXT_PUBLIC_URL || 'https://stacker.news')
    if (!allowedProtocols.has(parsed.protocol)) return 'about:blank'
  } catch {
    return 'about:blank'
  }
  return url
...
export function sanitizeMediaUrl (url) {
  return sanitizeUrl(url, MEDIA_URL_PROTOCOLS)
}

export function isSafeMediaUrl (url) {
  return !!url && sanitizeMediaUrl(url) !== 'about:blank'
}
...
export function assertContentTypeJson (res, { method } = {}) {
  ...
  // Parse the MIME type before parameters (e.g. `;charset=utf-8`) so we never
  // accept content types like `text/application/json` that happen to contain
  // the expected substring.
  const mimeType = contentType.split(';')[0].trim().toLowerCase()
  if (mimeType !== expected) {
    throw new ContentTypeAssertError(res, { method, expected, actual: mimeType || 'none' })
  }
}
