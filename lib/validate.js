...
export function bountySchema (args) {
  return object({
    ...
    name: 'text-or-lexical',
    test: (value) => !!(value.text || value.lexicalState),
    message: 'cannot submit empty text'
  }).test({
...
export function territorySchema (args) {
  return object({
    ...
    desc: string().trim().max(
      MAX_TERRITORY_DESC_LENGTH,
      ({ max, value }) => `-${Math.abs(max - value.length)} characters remaining`
    ),
    // we need to trim this too in some way
    lexicalState: string(),
    ...
    name: 'desc-or-lexical',
    test: (value) => !!(value.desc || value.lexicalState),
    message: 'territory must have a description'
  })
}
...
export const commentSchema = object({
  text: textValidator(MAX_COMMENT_TEXT_LENGTH),
  lexicalState: string()
}).test({
  name: 'text-or-lexical',
  test: (value) => !!(value.text || value.lexicalState),
...
export const tableSchema = object({
  rows: intValidator.required('required').positive('must be positive').min(1, 'must be at least 1').max(500, 'must be at most 500'),
  columns: intValidator.required('required').positive('must be positive').min(1, 'must be at least 1').max(50, 'must be at most 50')
})

export const customMigrationSchema = object({
  by: string().required('required').oneOf(['id', 'date', 'user'], 'must be one of: id, date, or user'),
  fromId: intValidator
    .when('by', {
      is: 'id',
      then: schema => schema.required('required').positive('must be positive').min(1, 'must be at least 1'),
  ...
  migrateComments: boolean().required('required').default(true),
  checkMedia: boolean().notRequired().default(false)
}).test({
  name: 'id-range-check',
  test: function (value) {
    if (value.by === 'id' && value.fromId && value.toId) {
      const diff = value.toId - value.fromId
      return diff <= BATCH_MAX_ITEMS_PER_JOB
    }
    return true
  },
  message: `can only migrate up to ${BATCH_MAX_ITEMS_PER_JOB} items at a time`
})
