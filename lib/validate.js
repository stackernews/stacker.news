...
export const youtubeClipMetaSchema = object({
  clipId: string().required('required').matches(/^[\w_]+$/, 'invalid clip id').max(64, 'clip id is too long'),
  clipt: string().matches(/^[\w_]+$/, 'invalid clipt').max(64, 'clipt is too long'),
  videoId: string().matches(/^[\w_]+$/, 'invalid video id').max(64, 'video id is too long')
})

export const youtubeClipSchema = object({
  id: string().required('required').matches(/^[\w_]+$/, 'invalid clip id').max(64, 'clip id is too long'),
  meta: youtubeClipMetaSchema
})
