...
export const lnbitsSchema = object().shape({
  ...
  adminKey: string().length(32).when('invoiceKey', {
    is: (key) => !key,
    then: (schema) => schema.required('required if invoice key not set')
  }),
  invoiceKey: string().length(32).when('adminKey', {
    is: (key) => !key,
    then: (schema) => schema.required('required if admin key not set')
  })
