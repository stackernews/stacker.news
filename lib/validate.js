...
export function territorySchema (args) {
  return object({
    ...
    commentsSatsFilter: intValidator
      .max(1000, 'must be at most 1000')
      .when('replyCost', ([replyCost], schema) =>
        schema.min(replyCost || 1, `must be at least the reply cost (${replyCost || 1})`)
      ),
...
export const settingsSchema = object().shape({
  ...
  postsSatsFilter: intValidator.required('required').min(-1000, 'must be at least -1000').max(1000, 'must be at most 1000'),
  commentsSatsFilter: intValidator.required('required').min(-1000, 'must be at least -1000').max(1000, 'must be at most 1000'),
