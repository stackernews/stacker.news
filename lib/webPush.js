...
export async function notifyDeposit (userId, invoice) {
  try {
    await sendUserNotification(userId, {
      title: `${numWithUnits(msatsToSats(invoice.msatsReceived), { abbreviate: false, unitSingular: 'bitcoin was', unitPlural: 'bitcoins were' })} deposited in your account`,
...
export async function notifyWithdrawal (wdrwl) {
  try {
    await sendUserNotification(wdrwl.userId, {
      title: `${numWithUnits(msatsToSats(wdrwl.msatsPaid), { abbreviate: false, unitSingular: 'bitcoin was', unitPlural: 'bitcoins were' })} withdrawn from your account`,
      tag: 'WITHDRAWAL',
