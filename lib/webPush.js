    ...
    const userSubsExcludingMutes = await models.$queryRaw`
      ...
      WHERE us."followeeId" = ${Number(item.userId)}::INTEGER
        ...
        ${isPost
          ...
          : Prisma.empty}
        -- satsFilter gating
        AND (follower."satsFilter" <= 0 OR (i.cost + i.boost + (i.msats / 1000)) >= follower."satsFilter")
    `
...
export async function notifyTerritorySubscribers ({ models, item }) {
  try {
    ...
    WHERE ss."subName" = $1
      ...
      AND (u."satsFilter" <= 0 OR (i.cost + i.boost + (i.msats / 1000)) >= u."satsFilter")
...
export async function notifyItemParents ({ models, item }) {
  try {
    ...
    const parents = await models.$queryRaw`
      ...
      -- satsFilter gating
      AND (u."satsFilter" <= 0 OR (ri.cost + ri.boost + (ri.msats / 1000)) >= u."satsFilter")`
    await Promise.allSettled(
