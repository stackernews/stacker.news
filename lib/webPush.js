...
function createPayload (notification) {
  ...
  return JSON.stringify({
    ...
    notification: {
      ...
      body,
      timestamp: Date.now(),
      navigate: process.env.NEXT_PUBLIC_URL, // navigate is required
      ...options
    },
    app_badge: 1
  })
...
async function createItemUrl (id) {
  ...
  return `${process.env.NEXT_PUBLIC_URL}/items/${rootItem.id}` + (rootItem.id !== id ? `?commentId=${id}` : '')
}
...
async function sendUserNotification (userId, notification) {
  try {
    ...
    if (notification.itemId) {
      ...
      notification.navigate ??= notification.data.url
