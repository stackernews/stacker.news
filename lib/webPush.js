...
function satsFilterClause (userTable, itemTable) {
  return `(
    CASE WHEN ${itemTable}."parentId" IS NULL
      THEN (${itemTable}.cost + ${itemTable}.boost + (${itemTable}.msats / 1000)) >= ${userTable}."satsFilter"
      ELSE (${itemTable}.cost + ${itemTable}.boost + (${itemTable}.msats / 1000)) >= LEAST(${userTable}."satsFilter", 1)
    END
  )`
}
...
export async function notifyUserSubscribers ({ models, item }) {
  try {
    ...
    const userSubsExcludingMutes = await models.$queryRawUnsafe(`
      ...
      INNER JOIN "Item" i ON i.id = $1::INTEGER
      WHERE "followeeId" = $2::INTEGER
        AND ${isPost ? '"postsSubscribedAt"' : '"commentsSubscribedAt"'} IS NOT NULL
        -- satsFilter check
        AND ${satsFilterClause('follower', 'i')}
        -- ignore muted users
        AND NOT EXISTS (
          SELECT 1
          FROM "Mute"
          WHERE "Mute"."muterId" = "UserSubscription"."followerId"
          AND "Mute"."mutedId" = $2::INTEGER)
        -- ignore subscription if user was already notified of item as a reply
        AND NOT EXISTS (
          SELECT 1 FROM "Reply"
          INNER JOIN users f ON f.id = "UserSubscription"."followerId"
          WHERE "Reply"."itemId" = $1::INTEGER
          AND "Reply"."ancestorUserId" = f.id
          AND f."noteAllDescendants"
        )
        ${isPost
          ...
          : ''}
    `, Number(item.id), Number(item.userId), item.subName)
...
export async function notifyTerritorySubscribers ({ models, item }) {
  try {
    ...
    const territorySubsExcludingMuted = await models.$queryRawUnsafe(`
      SELECT "SubSubscription"."userId"
      FROM "SubSubscription"
      INNER JOIN users u ON u.id = "SubSubscription"."userId"
      INNER JOIN "Item" i ON i.id = $1::INTEGER
      WHERE "subName" = $2
        AND "SubSubscription"."userId" <> $3::INTEGER
        -- satsFilter check
        AND ${satsFilterClause('u', 'i')}
        -- ignore muted users
        AND NOT EXISTS (
          SELECT 1 FROM "Mute" m 
          WHERE m."muterId" = "SubSubscription"."userId" 
          AND m."mutedId" = $3::INTEGER
        )
    `, Number(item.id), subName, Number(item.userId))
...
export async function notifyThreadSubscribers ({ models, item }) {
  try {
    ...
    const subscribers = await models.$queryRawUnsafe(`
      ...
      JOIN "Item" i ON i.id = $1::INTEGER
      WHERE r."itemId" = $1::INTEGER
      ...
      AND NOT EXISTS (
        SELECT 1 FROM "Item" ancestor
        JOIN "Item" p ON p.path @> ancestor.path
        WHERE ancestor.id = $2::INTEGER 
        AND p."userId" = "ThreadSubscription"."userId" 
        AND users."noteAllDescendants"
      )
    `, Number(item.id), Number(item.parentId))
...
export async function notifyItemParents ({ models, item }) {
  try {
    ...
    const parents = await models.$queryRawUnsafe(`
      ...
      JOIN "Item" notified ON notified.id = $1::INTEGER
      WHERE i.id = $2::INTEGER 
        AND p."userId" <> $3::INTEGER
        -- satsFilter check
        AND ${satsFilterClause('u', 'notified')}
        -- ignore muted users
        AND NOT EXISTS (
          SELECT 1 FROM "Mute" m
          WHERE m."muterId" = p."userId" AND m."mutedId" = $3::INTEGER
        )
        AND EXISTS (
    ...
    `, Number(item.id), Number(item.parentId), Number(user.id))
    await Promise.allSettled(
...
export async function notifyMention ({ models, userId, item }) {
  try {
    ...
    if (muted) return
    const [{ passesFilter }] = await models.$queryRawUnsafe(`
      SELECT ${satsFilterClause('u', 'i')} as "passesFilter"
      FROM users u, "Item" i
...
export async function notifyItemMention ({ models, referrerItem, refereeItem }) {
  try {
    ...
    const [{ passesFilter }] = await models.$queryRawUnsafe(`
      ...
      FROM users u, "Item" i
      WHERE u.id = $1 AND i.id = $2
    `, refereeItem.userId, referrerItem.id)
    if (!passesFilter) return
