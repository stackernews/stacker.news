...
async function meetsUserSatFilter (userId, item) {
  ...
  const filter = isPost
    ? (user?.postsSatsFilter ?? DEFAULT_POSTS_SATS_FILTER)
    : (user?.commentsSatsFilter ?? DEFAULT_COMMENTS_SATS_FILTER)
...
export async function notifyUserSubscribers ({ models, item }) {
  try {
    ...
    // Filter by each user's sat filter preferences
    const filteredSubs = await Promise.all(
      userSubsExcludingMutes.map(async (sub) => ({
        ...sub,
        meetsFilter: await meetsUserSatFilter(sub.followerId, item)
      }))
    )
  ...
  try {
    ...
    const filteredSubs = await Promise.all(
      territorySubsExcludingMuted
        ...
        .map(async (sub) => ({
          ...sub,
          meetsFilter: await meetsUserSatFilter(sub.userId, item)
        }))
    )
...
export async function notifyThreadSubscribers ({ models, item }) {
  try {
    ...
    // Filter by each user's sat filter preferences
    const filteredSubscribers = await Promise.all(
...
export async function notifyItemParents ({ models, item }) {
  try {
    ...
    // Filter by each user's sat filter preferences
    const filteredParents = await Promise.all(
      parents.map(async (parent) => ({
        ...parent,
        meetsFilter: await meetsUserSatFilter(parent.userId, item)
      }))
    )
