...
async function sendUserNotification (userId, notification) {
  try {
    ...
    // remove transient id after navigation enrichment
...
export async function notifyUserSubscribers ({ models, item }) {
  try {
    ...
    const userSubsExcludingMutes = await models.$queryRaw`
      SELECT us."followerId",
             us."followeeId",
             author.name as "followeeName"
      FROM "UserSubscription" us
      INNER JOIN users author ON author.id = us."followeeId"
      INNER JOIN users follower ON follower.id = us."followerId"
      INNER JOIN "Item" i ON i.id = ${Number(item.id)}::INTEGER
      WHERE us."followeeId" = ${Number(item.userId)}::INTEGER
        AND ${isPost ? Prisma.sql`us."postsSubscribedAt"` : Prisma.sql`us."commentsSubscribedAt"`} IS NOT NULL
        -- ignore muted users
        AND NOT EXISTS (
          SELECT 1
          FROM "Mute"
          WHERE "Mute"."muterId" = us."followerId"
            AND "Mute"."mutedId" = ${Number(item.userId)}::INTEGER
        )
        -- ignore subscription if user was already notified of item as a reply
        AND NOT EXISTS (
          SELECT 1 FROM "Reply"
          INNER JOIN users f2 ON f2.id = us."followerId"
          WHERE "Reply"."itemId" = ${Number(item.id)}::INTEGER
            AND "Reply"."ancestorUserId" = f2.id
            AND f2."noteAllDescendants"
        )
        ...
        ${isPost
          ? Prisma.sql`AND NOT EXISTS (
              ...
              WHERE "SubSubscription"."userId" = us."followerId"
                AND "SubSubscription"."subName" = ${item.subName}
            )`
          : Prisma.empty}
        -- satsFilter gating (DB-level)
        AND ${
          isPost
            ? Prisma.sql`(COALESCE(follower."satsFilter", 0) <= 0
                          OR (i.cost + i.boost + (i.msats / 1000)) >= follower."satsFilter")`
            : Prisma.sql`(LEAST(COALESCE(follower."satsFilter", 0), 1) <= 0
                          OR (i.cost + i.boost + (i.msats / 1000)) >= LEAST(COALESCE(follower."satsFilter", 0), 1))`
        }
    `
...
export async function notifyTerritorySubscribers ({ models, item }) {
  try {
    ...
    SELECT ss."userId"
    FROM "SubSubscription" ss
    JOIN users u ON u.id = ss."userId"
    JOIN "Item" i ON i.id = $2::INTEGER
    WHERE ss."subName" = $1
      AND NOT EXISTS (SELECT 1 FROM "Mute" m WHERE m."muterId" = ss."userId" AND m."mutedId" = $3)
      AND (COALESCE(u."satsFilter", 0) <= 0 OR (i.cost + i.boost + (i.msats / 1000)) >= u."satsFilter")
    `, subName, Number(item.id), Number(item.userId))
...
export async function notifyThreadSubscribers ({ models, item }) {
  try {
    ...
    const subscribers = await models.$queryRaw`
      ...
      JOIN "Reply" r ON ts."itemId" = r."ancestorId"
      JOIN "Item" i ON i.id = r."itemId"
      WHERE r."itemId" = ${item.id}
      -- don't send notifications for own items
      AND r."userId" <> ts."userId"
      -- send notifications for all levels?
      AND CASE WHEN u."noteAllDescendants" THEN TRUE ELSE r.level = 1 END
      -- muted?
      AND NOT EXISTS (SELECT 1 FROM "Mute" m WHERE m."muterId" = u.id AND m."mutedId" = r."userId")
      -- already received notification as reply to self?
      AND NOT EXISTS (
        SELECT 1 FROM "Item" i2
        JOIN "Item" p ON p.path @> i2.path
        WHERE i2.id = ${item.parentId} AND p."userId" = ts."userId" AND u."noteAllDescendants"
      )
      -- satsFilter gating for comments
      AND (LEAST(COALESCE(u."satsFilter", 0), 1) <= 0 OR (i.cost + i.boost + (i.msats / 1000)) >= LEAST(COALESCE(u."satsFilter", 0), 1))`
...
export async function notifyItemParents ({ models, item }) {
  try {
    ...
    const parents = await models.$queryRaw`
      ...
      -- satsFilter gating for comments (replies)
      AND (LEAST(COALESCE(u."satsFilter", 0), 1) <= 0 OR (ri.cost + ri.boost + (ri.msats / 1000)) >= LEAST(COALESCE(u."satsFilter", 0), 1))`
    await Promise.allSettled(
