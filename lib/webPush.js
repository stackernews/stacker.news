...
const userSubsExcludingMutes = await models.$queryRaw`
  SELECT "UserSubscription"."followerId",
         "UserSubscription"."followeeId",
         users.name as "followeeName"
  FROM "UserSubscription"
  INNER JOIN users ON users.id = "UserSubscription"."followeeId"
  INNER JOIN users ON users.id = "UserSubscription"."followerId"
  INNER JOIN "Item" ON "Item".id = ${Number(item.id)}::INTEGER
  WHERE "UserSubscription"."followeeId" = ${Number(item.userId)}::INTEGER
    AND ${isPost ? Prisma.sql`"UserSubscription"."postsSubscribedAt"` : Prisma.sql`"UserSubscription"."commentsSubscribedAt"`} IS NOT NULL
    -- ignore muted users
    AND NOT EXISTS (
      SELECT 1
      FROM "Mute"
    ...
    AND NOT EXISTS (
      ...
      AND "Reply"."ancestorUserId" = users.id
    ...
    ${isPost
      ...
      : Prisma.empty}
    -- satsFilter gating
    AND (users."satsFilter" <= 0 OR ("Item".cost + "Item".boost + ("Item".msats / 1000)) >= users."satsFilter")
`
...
export async function notifyTerritorySubscribers ({ models, item }) {
  try {
    ...
    JOIN users ON users.id = "SubSubscription"."userId"
    JOIN "Item" i ON i.id = $2::INTEGER
    WHERE "SubSubscription"."subName" = $1
      AND NOT EXISTS (SELECT 1 FROM "Mute" m WHERE m."muterId" = "SubSubscription"."userId" AND m."mutedId" = $3)
      AND (users."satsFilter" <= 0 OR (i.cost + i.boost + (i.msats / 1000)) >= users."satsFilter")
...
export async function notifyThreadSubscribers ({ models, item }) {
  try {
    ...
    const subscribers = await models.$queryRaw`
      SELECT DISTINCT "ThreadSubscription"."userId"
      ...
      AND NOT EXISTS (
        ...
        WHERE i2.id = ${item.parentId} AND p."userId" = "ThreadSubscription"."userId" AND users."noteAllDescendants"
      )
      -- satsFilter gating
      AND (users."satsFilter" <= 0 OR (i.cost + i.boost + (i.msats / 1000)) >= users."satsFilter")`
  ...
  try {
    ...
    const parents = await models.$queryRaw`
      ...
      AND EXISTS (
        ...
        SELECT 1 FROM "ThreadSubscription" "ThreadSubscription" WHERE p.id = "ThreadSubscription"."itemId" AND p."userId" = "ThreadSubscription"."userId"
      )
      -- satsFilter gating
      AND (users."satsFilter" <= 0 OR (ri.cost + ri.boost + (ri.msats / 1000)) >= users."satsFilter")`
    await Promise.allSettled(
