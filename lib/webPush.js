...
function userFilterFragment (setting) {
  return setting ? { user: { [setting]: true } } : {}
}

function satsFilterWhere (userAlias = 'u', itemAlias = 'i') {
  return `(${userAlias}."satsFilter" <= 0 OR (${itemAlias}.cost + ${itemAlias}.boost + (${itemAlias}.msats / 1000)) >= ${userAlias}."satsFilter")`
}
    ...
    .catch(async (err) => {
      switch (err.statusCode) {
        ...
        case 410: {
          ...
          const deletedSubscription = await models.pushSubscription.delete({ where: { id } })
          log(`deleted subscription ${id} of user ${deletedSubscription.userId} due to push error`)
          break
        }
        case 413:
  ...
  try {
    ...
    const userSubsExcludingMutes = await models.$queryRaw`
      ...
      WHERE us."followeeId" = ${Number(item.userId)}::INTEGER
        ...
        AND ${Prisma.raw(satsFilterWhere('follower', 'i'))}
    `
...
export async function notifyThreadSubscribers ({ models, item }) {
  try {
    ...
    const subscribers = await models.$queryRaw`
      ...
      -- satsFilter gating
      AND ${Prisma.raw(satsFilterWhere('u', 'i'))}`
...
export async function notifyItemParents ({ models, item }) {
  try {
    ...
    const parents = await models.$queryRaw`
      ...
      -- satsFilter gating
      AND ${Prisma.raw(satsFilterWhere('u', 'ri'))}`
    await Promise.allSettled(
