-- AlterEnum
ALTER TYPE "PostType" ADD VALUE 'EVENT';

-- AlterTable
ALTER TABLE "Item" ADD COLUMN "eventDate" TIMESTAMP(3);
ALTER TABLE "Item" ADD COLUMN "eventLocation" TEXT;
DROP FUNCTION IF EXISTS create_item(
    sub TEXT, title TEXT, url TEXT, text TEXT, event_date TIMESTAMP, event_location TEXT, boost INTEGER, 
...
BEGIN
    ...
    INSERT INTO "Item"
    ("subName", title, url, text, "eventDate", "eventLocation", "userId", "parentId", "fwdUserId",
        freebie, "weightedDownVotes", created_at, updated_at)
    VALUES
    (sub, title, url, text, event_date, event_location, user_id, parent_id, fwd_user_id,
...
DROP FUNCTION IF EXISTS update_item(
    sub TEXT, item_id INTEGER, item_title TEXT, item_url TEXT, item_text TEXT, event_date TIMESTAMP, event_location TEXT, boost INTEGER,
    fwd_user_id INTEGER);
...
BEGIN
    ...
    SET "subName" = sub, title = item_title, url = item_url,
        text = item_text, "eventDate" = event_date, "eventLocation" = event_location, "fwdUserId" = fwd_user_id
    WHERE id = item_id
    RETURNING * INTO item;
