...
// Postgres int8 ceiling: larger amounts are provider garbage that would throw at write
...
export function walletAmountToMsats (amount) {
  ...
  // declared-unit wrapper: convert via the sats helper so .sat semantics have one owner
  if (amount?.sat != null) return BigInt(walletAmountToSats(amount)) * 1000n
...
function nonNegativeBigInt (amount) {
  ...
  if (value < 0n || value > PG_INT8_MAX) throw new Error('invalid wallet amount')
  return value
}
